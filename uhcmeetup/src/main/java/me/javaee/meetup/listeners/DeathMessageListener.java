...
public class DeathMessageListener implements Listener {
    ...
    private String getDisplayName(final Entity entity) {
        ...
        if(entity instanceof Player) {
            ...
            return ProfileUtils.getInstance().calculateColor(player) + "[" + ProfileUtils.getInstance().calculateRank(player).charAt(0) + "]" + ChatColor.RED + player.getName() + ChatColor.GRAY + '[' + ChatColor.WHITE + ProfileUtils.getInstance().getProfile(player.getUniqueId()).getMatchKills() + ChatColor.GRAY + ']';
        }
