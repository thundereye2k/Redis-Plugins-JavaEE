...
import me.redis.kohi.timer.TimerCooldown;
...
public class EnderpearlTimer extends PlayerTimer implements Listener {
    ...
    public void onExpire(UUID userUUID) {
        ...
        if (player != null) {
            player.sendMessage(ChatColor.GREEN + "Your enderpearl timer has expired. You may now Enderpearl again.");
        }
    }
        ...
        if (event.getItem() != null && event.getItem().getType() == Material.ENDER_PEARL) {
            if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
                ...
                if (remaining > 0L) {
                    ...
                    player.sendMessage(ChatColor.RED + "You still have an enderpearl cooldown for another " + ChatColor.BOLD + DurationFormatter.getRemaining(remaining, true, false) + ChatColor.RED + '.');
                }
            }
        }
    }
