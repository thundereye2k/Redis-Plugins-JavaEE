...
import me.redis.practice.events.PlayerCreateTeamEvent;
...
public class TeamCreateCommand extends CommandArgument {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (profile.getStatus() != ProfileStatus.LOBBY) {
                player.sendMessage(ChatColor.RED + "You must be in the spawn to create a team.");
                return true;
            }
            ...
            PlayerCreateTeamEvent playerCreateTeamEvent = new PlayerCreateTeamEvent(player);
            playerCreateTeamEvent.setTeam(team);
            Bukkit.getPluginManager().callEvent(playerCreateTeamEvent);
